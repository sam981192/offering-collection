...
function getSafeElement(id) {
  ...
  if (!el) {
    const errorMsg = `DOM Element with id "${id}" is missing from the page.`;
...
function countValidLines(text) {
  ...
  for (let line of lines) {
    ...
    if (SKIP_SET.has(lcRaw) || SKIP_SET.has(trimmed.toLowerCase())) continue;
    const cleaned = cleanLine(trimmed);
    if (!cleaned) continue;
    const cleanedLc = cleaned.toLowerCase();
    if (SKIP_SET.has(cleanedLc)) continue;
    count++;
  }
  return count;
}
...
['input1', 'input2', 'input3'].forEach((id, i) => {
  try {
    ...
    console.error(`Failed to initialize listener for #${id}:`, err);
  }
});
...
function processInputs(t1, t2, t3) {
  ...
  function processLine(raw, forceOutside, source) {
    ...
    // Flag for admin review: only if contains/fuzzy, or unmatched from general inputs
...
function showChartErrorState(message) {
  const errMsg = `Charts unavailable: ${message}`;
  toggleChartState('barChart', 'barChartEmpty', false, errMsg);
  toggleChartState('pieChart', 'pieChartEmpty', false, errMsg);
  toggleChartState('outsideChart', 'outsideChartEmpty', false, errMsg);
}
...
function buildCharts(catCount, subCount) {
  ...
  if (typeof Chart === 'undefined') {
    const msg = 'Charts unavailable, but report generated.';
  ...
  if (top15.length) {
    ...
    toggleChartState('outsideChart', 'outsideChartEmpty', false, 'No Outside Delhi entries.');
  }
}
...
function calculateReport() {
  try {
    ...
    // Render cards, final report, breakdown, and admin review first
    try {
      ...
      console.error('Error rendering report sections:', renderError);
      showToast(`⚠️ Render error: ${renderError.message}`);
    }

    // Render charts safely wrapped to prevent blocking the report display
    try {
    ...
    getSafeElement('btn-excel').disabled = !xlsxAvailable;
    getSafeElement('btn-pdf').disabled = !jspdfAvailable;
...
function copyFullBreakdown() {
  if (!lastResult) return;
  const el = getSafeElement('full-breakdown');
...
function downloadExcel() {
  if (typeof XLSX === "undefined") {
    showToast("❌ Excel export library (XLSX) failed to load.");
    return;
  }
  if (!lastResult) return;
...
function downloadPDF() {
  if (typeof window.jspdf === "undefined") {
    showToast("❌ PDF export library (jsPDF) failed to load.");
    return;
  }
  if (!lastResult) return;
  ...
  if (typeof doc.autoTable !== "function") {
    showToast("❌ PDF Table plugin (autoTable) failed to load.");
    return;
  }
